export type SongSectionType =
  | 'intro'
  | 'verse'
  | 'pre-chorus'
  | 'chorus'
  | 'bridge'
  | 'interlude'
  | 'outro'
  | 'tag'
  | 'instrumental'

export type SongSection = {
  id: string
  type: SongSectionType
  label: string
  lyrics: string
  sameAs?: string
  repeat?: string
}

export type SongMedia = {
  youtubeUrl?: string
  spotifyUrl?: string
}

export type Song = {
  id: string
  title: string
  subtitle?: string
  artist: string
  album?: string
  songwriter?: string
  language: string
  key: string
  songType: string
  sections: SongSection[]
  media?: SongMedia
}

export const songs: Song[] = [
  {
    id: "bersyukurlah-live",
    title: "Bersyukurlah",
    artist: "JPCC Worship",
    album: "Take Us Higher (JPCC Worship) [Live Recording]",
    songwriter: "True Worshippers",
    language: "id",
    key: "E",
    songType: "Upbeat (Praise)",
    sections: [
      {
        id: "bersyukurlah-intro",
        type: "intro",
        label: "INTRO",
        lyrics: "E              B               C#m\nNana nana nana,  nana nana nana \n               A\nNana nana nana na (yee yee yee) \nE              B               C#m\nNana nana nana,  nana nana nana \n               A\nNana nana nana na. ",
      },
      {
        id: "bersyukurlah-verse-1",
        type: "verse",
        label: "VERSE",
        lyrics: "E               B\n  Haleluya ku memujiMu Tuhan \nC#m                  A\n  Kar’na kasihMu tak berkesudahan \nE                      B\n  Biar yang bernafas memuji Sang Raja \nD                       B\n  Di atas gunung dan di dalam samudra. ",
      },
      {
        id: "bersyukurlah-chorus",
        type: "pre-chorus",
        label: "Pre-Chorus",
        lyrics: "A           E\nTanganku ku angkat padaMu \nG#m        A                   E/G#\nMensyukuri rahmat yang Kau b’ri \nA        E\nKakiku melompat bagiMu \nF#             B               A  • A  B\nS’bab rahmatMu baru s’tiap pagi. ",
      },
      {
        id: "bersyukurlah-interlude",
        type: "chorus",
        label: "Chorus",
        lyrics: "E            B\nBersyukurlah kepada Tuhan \nC#m             A        C#m  B\nSebab Dia baik, sebab Ia baik \nE          B\nBahwasanya untuk selamanya \nC#m             A         C#m B\nKasih setiaNya, kasih setia - Nya. ",
      },
      {
        id: "verse-1784138326054-f5cf45100b76f",
        type: "chorus",
        label: "Chorus",
        lyrics: "| B  •  B  C# |\n--- \nF#           C#\nBersyukurlah kepada Tuhan \nD#m             B        D#m   C#\nSebab Dia baik, sebab Ia baik \nF#         C#\nBahwasanya untuk selamanya \nD#m             B         D#m C#\nKasih setiaNya, kasih setia - Nya. ",
        repeat: "(Overtune)",
      },
    ],
  },
  {
    id: "hatiku-percaya",
    title: "Hatiku percaya",
    artist: "True Worshippers",
    album: "Shine Like Stars",
    songwriter: "True Worshippers",
    language: "id",
    key: "F",
    songType: "Worship",
    sections: [
      {
        id: "hatiku-percaya-verse",
        type: "intro",
        label: "Intro",
        lyrics: "| F      .     Bb      .    |\n|   Eb    Bb  Dm    C     F |",
      },
      {
        id: "hatiku-percaya-pre-chorus",
        type: "verse",
        label: "Verse",
        lyrics: "F              Bb\n Tuhanlah kekuatan dan mazmurku\nGm                   Bb           C\n Dia gunung batu dan kes'lamatanku\nF               Bb\n Hanya padaMu hatiku percaya\nGm             Eb      Bb         C\n Kaulah menara,  dan kota perlindungan.",
      },
      {
        id: "hatiku-percaya-chorus",
        type: "chorus",
        label: "CHORUS",
        lyrics: "F                    Bb\n Ku mau s'lalu bersyukur\n                   Dm\n S'bab cintaMu padaku\n                    Bb\n Tak'kan pernah berubah\n   C\n Hatiku percaya\nF                 C/E\n Walau bumi bergoncang\n                    Dm\n Gunung-gunung beranjak\n                  Bb\n Namun kasih setiaMu\n     C         F\n Tak pergi dariku",
      },
      {
        id: "hatiku-percaya-interlude",
        type: "interlude",
        label: "Interlude",
        lyrics: "| Dm7  Dm6  Dm7  Dm6 |\n| Bb   C    Eb   Bb  |\n| Dm7  Dm6  Dm7  Dm6 |\n| Bb   C    Eb   Bb  |\n| G#   Bb   Bb   C   |",
      },
      {
        id: "hatiku-percaya-verse-2",
        type: "chorus",
        label: "Chorus",
        lyrics: "| Bb  Bb  C  D |\n---\nG                   C\nKu mau s'lalu bersyukur\n                  Em\nS'bab cintaMu padaku\n                   C\nTak'kan pernah berubah\n  D         G\nHatiku percaya\nG                D/Gb\nWalau bumi bergoncang\n                   Em\nGunung-gunung beranjak\n                 C\nNamun kasih setiaMu\n    D         Em\nTak pergi dariku.",
        repeat: "(Overtune)",
      },
      {
        id: "hatiku-percaya-outro",
        type: "outro",
        label: "Outro",
        lyrics: "| G      .      C      .   | \n|   F     C   Em    D      |\n|--------------------------|x2\n|  G  C  •  F | C  •  G  • |",
        repeat: "(Overtune)",
      },
    ],
  },
  {
    id: "kuberikan-hatiku",
    title: "Kuberikan Hatiku",
    subtitle: "Live",
    artist: "Franky Sihombing",
    album: "Rayakan 1",
    songwriter: "Franky Sihombing",
    language: "id",
    key: "F",
    songType: "Slowbeat (Worship)",
    sections: [
      {
        id: "kuberikan-hatiku-verse",
        type: "verse",
        label: "VERSE",
        lyrics: "F          Dm    Bb F  C\nInilah rinduku kepa-da-Mu \nDm           F          Eb  Bb  C\nSegenap hatiku menyembahMu \nF           Dm    Bb F  C\nSeluruh hidupku memu-ji-Mu \nDm       F          Eb  Bb  C\nKu memujaMu ya Allahku \n",
      },
      {
        id: "kuberikan-hatiku-bridge",
        type: "verse",
        label: "Verse",
        lyrics: "F          Dm    Bb F  C\nInilah rinduku kepa-da-Mu \nDm          F          Eb  Bb  C\nSegnap hatiku menyembahMu \nF           Dm    Bb F  C\nSeluruh jiwaku memu-ji-Mu \nDm       F          Eb  Bb  C\nKu memujaMu ya Allahku \n",
      },
      {
        id: "kuberikan-hatiku-outro",
        type: "chorus",
        label: "Chorus",
        lyrics: "F             C          Gm\nKuberikan hatiku dan jiwaku \n  Bb     C     F\nSemuanya bagiMu \n              C               Gm\nDi dalam hidupku di setiap waktu \n   Bb    C    F\nNyatakan jalanMu ",
      },
      {
        id: "verse-1784361360682-bddda991136988",
        type: "chorus",
        label: "Chorus",
        lyrics: "G             D          Am\nKuberikan hatiku dan jiwaku \n  C      D     G\nSemuanya bagiMu \n              D               Am\nDi dalam hidupku di setiap waktu \n   C     D    G\nNyatakan jalanMu \n",
        repeat: "(Overtune)",
      },
    ],
  },
]